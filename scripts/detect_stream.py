...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run YOLOv8 weapon detection on an RTSP/HTTP stream.")
    parser.add_argument(
        "--source",
        type=str,
        required=True,
        help="RTSP or HTTP CCTV stream URL.",
    )
    ...
    parser.add_argument("--alert-webhook", help="Approved HTTPS dispatch endpoint. Alerts remain local if omitted.")
    ...
    parser.add_argument("--conf", type=float, default=0.90, help="Confidence threshold. Use 0.90+ for safety alerts.")
    ...
    parser.add_argument(
        "--no-save",
        action="store_true",
        help="Disable saving the annotated stream video.",
    )
    parser.add_argument(
        "--name",
        type=str,
...
def main() -> None:
    ...
    output_dir = make_output_dir("detect_stream", args.name, "outputs")
    alert_manager = AlertManager(
        ...
        webhook_token=os.environ.get("WEAPON_ALERT_TOKEN"),
    ...
    output_path = process_video_source(
        ...
        window_name="Weapon CCTV Detection",
        alert_manager=alert_manager,
    )

    if output_path is not None:
        print(f"Saved annotated stream video to {output_path}")
    else:
        print("Stream processed without saving an output file.")
